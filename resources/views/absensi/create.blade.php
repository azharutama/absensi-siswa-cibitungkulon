<x-app-layout>
    {{-- Filter menentukan kelas dan tanggal sebelum form absensi ditampilkan. --}}
    <div class="p-6 space-y-6">
        ...
        @if(auth()->user()->role === 'guru')
            <x-absensi-filter :action="route('absensi.create')" :kelas="$kelas" :kelasId="$kelasId" :tanggal="$tanggal" :disabled="$periodeWarning !== null" :hideKelas="true" :activeDates="$activeDates ?? []" />
        @else
            <x-absensi-filter :action="route('absensi.create')" :kelas="$kelas" :kelasId="$kelasId" :tanggal="$tanggal" :disabled="$periodeWarning !== null" :activeDates="$activeDates ?? []" />
        @endif

            @if(false && $periodeWarning)
                <div class="p-4 bg-red-100 border-l-4 border-red-500 text-red-700 rounded shadow-sm flex items-start gap-3">
                    <svg class="w-5 h-5 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54-1.333 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694 0-3.464 0L3.34 16c-.77-1.333-.192 3 1.732 3z"/>
                    </svg>
                    <div>
                        <p class="font-semibold">Periode Belum Dikonfigurasi</p>
                        <p class="mt-1">{{ $periodeWarning }}</p>
        ...
        @endif
            ...
            @endif
        {{-- Form hanya tersedia jika kelas valid, memiliki siswa, dan tanggal merupakan hari aktif. --}}
