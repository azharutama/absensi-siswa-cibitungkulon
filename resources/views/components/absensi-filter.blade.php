...
<div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm mb-6">
    <form method="GET" action="{{ $action }}" class="flex flex-wrap items-end gap-4">
        ...
        <div class="w-full sm:w-48">
            ...
            <input id="absensi-tanggal" type="date" name="tanggal" value="{{ $tanggal ?? date('Y-m-d') }}" max="{{ today()->toDateString() }}" {{ $disabled ? 'disabled' : '' }} onchange="this.form.submit()" class="w-full bg-gray-50 border border-gray-200 rounded-lg text-sm px-3 py-2 text-gray-700 focus:ring-blue-500 focus:border-blue-500 disabled:opacity-50 disabled:cursor-not-allowed">
        </div>
    </form>
</div>
